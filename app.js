(function(){
  ...
  function renderTable(stations){
    ...
    if(stations.length === 0){
      ...
      row.innerHTML = `<td colspan="4" style="text-align:center">Aucune station disponible</td>`;
    ...
    stations.forEach(s=>{
      ...
      tr.innerHTML = `
        <td>${s.name}</td>
        <td>${s.city}</td>
        <td>${s.updated ? new Date(s.updated).toLocaleString() : '—'}</td>
        <td>${statut}</td>
      `;
  ...
  toggleMapBtn.addEventListener('click', ()=>{ mapEl.classList.toggle('map-hidden'); });
  ...
  themeToggle.addEventListener('click', ()=>{ const cur = localStorage.getItem('carbu_theme')||'light'; const next = cur